...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .futebol-historico-resumo-stats {
      background: #1a1a1a;
      border: 1px solid #2a2a2a;
      border-radius: 10px;
      padding: 12px 14px;
    ...
    .futebol-historico-resumo-stats h4 {
      font-size: 14px;
      color: #fff;
      margin-bottom: 8px;
    }
    .futebol-historico-resumo-stats p {
      margin: 4px 0;
    }
    .futebol-historico-resumo-stats strong {
      color: #eee;
    }
    .futebol-gols-por-minuto {
      display: flex;
      flex-wrap: wrap;
      gap: 6px 14px;
      margin-top: 8px;
    ...
    .futebol-gol-minuto-wrap {
      position: relative;
      display: inline-block;
    }
    .futebol-gol-minuto-btn {
      background: transparent;
      border: none;
      padding: 0;
      font: inherit;
    ...
    .futebol-gol-minuto-btn.com-gol:hover,
    .futebol-gol-minuto-btn.com-gol:focus-visible {
      text-decoration-color: #ffb74d;
      outline: none;
    }
    .futebol-gol-minuto-popover {
      position: absolute;
      bottom: calc(100% + 6px);
      left: 0;
      min-width: 220px;
    ...
    .futebol-gol-minuto-popover ul {
      margin: 0;
      padding: 0;
      list-style: none;
    }
    .futebol-gol-minuto-popover li {
      padding: 5px 0;
      font-size: 12px;
      line-height: 1.35;
      border-bottom: 1px solid #2a2a2a;
    }
    .futebol-gol-minuto-popover li:last-child {
      border-bottom: none;
    }
    .futebol-gol-minuto-popover .gol-qtd {
      color: #ffb74d;
      font-weight: 600;
      margin-left: 4px;
    }
    ...
    .futebol-filtro-odd-custom input {
      width: 72px;
      background: #252525;
      border: 1px solid #333;
      border-radius: 8px;
    ...
    .futebol-filtro-odd-custom button {
      background: #3d2800;
    ...
    .futebol-odd-inicio-janela {
      color: #9ccc65;
      font-size: 11px;
      font-weight: 500;
      margin-left: 4px;
    }
    .futebol-estimativa-lucro {
      margin-top: 16px;
      padding-top: 14px;
      border-top: 1px solid #333;
    }
    .futebol-estimativa-lucro h4 {
      color: #ddd;
      font-size: 14px;
      margin-bottom: 6px;
    }
    .futebol-estimativa-lucro .positivo { color: #81c784; font-weight: 600; }
    .futebol-estimativa-lucro .negativo { color: #e57373; font-weight: 600; }
...
<body>
  ...
  <script type="module">
    ...
    const CHAVE_FUTEBOL_FILTRO_ODD_MIN = 'betano_futebol_filtro_odd_min';
    ...
    const filtroOddMinSalvo = localStorage.getItem(CHAVE_FUTEBOL_FILTRO_ODD_MIN);
    let futebolFiltroOddMinima = filtroOddMinSalvo === '' || filtroOddMinSalvo == null
      ? null
      : Number(filtroOddMinSalvo);
    if (futebolFiltroOddMinima != null && (!Number.isFinite(futebolFiltroOddMinima) || futebolFiltroOddMinima <= 0)) {
      futebolFiltroOddMinima = null;
    }
    ...
    async function buscarEstatisticasFutebol() {
      const [
        { data: partidas, error },
        { data: agenda },
        { data: resumoJanelaRaw, error: resumoErr },
        { data: eventosGol, error: eventosErr },
      ] = await Promise.all([
        supabase
    ...
    function normalizarResumoJanelaDb(raw) {
      const golsPorMinuto = {};
      const fonte = raw?.golsPorMinuto ?? {};
      for (const [k, v] of Object.entries(fonte)) {
        golsPorMinuto[Number(k)] = Number(v);
      }
      const resumo = {
        jogosColetados: Number(raw?.jogosColetados ?? 0),
        jogosComGolNaJanela: Number(raw?.jogosComGolNaJanela ?? 0),
        totalGolsNaJanela: Number(raw?.totalGolsNaJanela ?? 0),
        golsPorMinuto,
        maxMinutoComGol: Number(raw?.maxMinutoComGol ?? 87),
      };
      // #region agent log
      fetch('http://127.0.0.1:7904/ingest/86615625-6ae5-4e98-a1da-0a5f0f15fc42', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          runId: 'gols-db-rpc',
          hypothesisId: 'DB',
          location: 'historicoWebPage.ts:normalizarResumoJanelaDb',
          message: 'resumo janela via RPC',
          data: resumo,
          timestamp: Date.now(),
        }),
      }).catch(() => {});
    ...
    function montarGolsPorMinutoJogos(eventos) {
      const porMinuto = {};
      for (const ev of eventos ?? []) {
        const min = Number(ev.minuto_jogo);
        if (!Number.isFinite(min) || min < 85) continue;
        const partida = ev.futebol_partidas;
        if (!partida || !ev.partida_id) continue;
        ...
        porMinuto[min][pid].gols += Number(ev.quantidade ?? 0);
      }
      const resultado = {};
      for (const min of Object.keys(porMinuto)) {
        resultado[Number(min)] = Object.values(porMinuto[min]).sort((a, b) =>
          String(a.casa).localeCompare(String(b.casa), 'pt-BR'),
        );
      }
      return resultado;
    }
    ...
    function oddManterInicioJanela(partida, leituras) {
      if (!leituras?.length) return null;
      const odd = Number(leituras[0].odd_manter_placar);
      return Number.isFinite(odd) && odd > 0 ? odd : null;
    }

    function partidaPassaFiltroOddMinima(partida, leituras, oddMinima) {
      if (oddMinima == null) return true;
      const odd = oddManterInicioJanela(partida, leituras);
      if (odd == null) return false;
      return odd > oddMinima;
    }

    function partidaPassaFiltrosFutebol(partida, leituras, filtros) {
      return partidaPassaFiltroMaxGols(partida, filtros.maxGols)
        && partidaPassaFiltroOddMinima(partida, leituras, filtros.oddMinima);
    }
    ...
    function jogoAoVivoPassaFiltros(jogo, partidas, leiturasMap, filtros) {
      if (filtros.maxGols == null && filtros.oddMinima == null) return true;
      const partida = findPartidaParaJogoAoVivo(jogo, partidas);
      if (partida) {
        const leituras = leiturasMap[partida.id] ?? [];
        return partidaPassaFiltrosFutebol(partida, leituras, filtros);
      }
      if (jogo.em_janela) return false;
      if (filtros.oddMinima != null) return false;
      return jogoAoVivoPassaFiltroMaxGols(jogo, partidas, filtros.maxGols);
    }

    function montarHintFiltrosFutebol(filtroMeta) {
      if (!filtroMeta) {
        return 'Combine os filtros. Gols = placar aos 85\'. ODD = primeira coleta intensiva (mercado manter placar).';
      }
      const partes = [];
      if (filtroMeta.maxGols != null) partes.push('máx. ' + filtroMeta.maxGols + ' gols aos 85\'');
      if (filtroMeta.oddMinima != null) partes.push('ODD manter > ' + filtroMeta.oddMinima);
      if (!partes.length) {
        return 'Combine os filtros. Gols = placar aos 85\'. ODD = primeira coleta intensiva (mercado manter placar).';
      }
      return 'Filtros ativos: ' + partes.join(' E ') + ' · ' +
        filtroMeta.totalDepois + ' de ' + filtroMeta.totalAntes + ' jogos na janela';
    }

    function mensagemHistoricoVazioFiltro(filtroMeta) {
      if (!filtroMeta || (filtroMeta.maxGols == null && filtroMeta.oddMinima == null)) {
        return 'Nenhuma partida na janela final ainda. Com o monitor ativo, jogos ≥85\' aparecem aqui com coletas a cada 40–50 s.';
      }
      const partes = [];
      if (filtroMeta.maxGols != null) partes.push('até ' + filtroMeta.maxGols + ' gols aos 85\'');
      if (filtroMeta.oddMinima != null) partes.push('ODD manter > ' + filtroMeta.oddMinima);
      return 'Nenhuma partida na janela com ' + partes.join(' e ') + '.';
    }
    ...
    function partidaComGolNaJanelaEstimativa(partida) {
      if (partida.gols_na_janela != null && Number.isFinite(Number(partida.gols_na_janela))) {
        return Number(partida.gols_na_janela) > 0;
      }
      if (partida.gol_nos_ultimos_5_min === true) return true;
      if (partida.gol_nos_ultimos_5_min === false) return false;
      return null;
    }

    function calcularEstimativaLucroFutebol(historicoJanela, leiturasPorPartida) {
      let ganho = 0;
      let perdas = 0;
      let vencedores = 0;
      let perdedores = 0;
      ...
      for (const p of historicoJanela ?? []) {
        const odd = oddManterInicioJanela(p, leiturasPorPartida[p.id] ?? []);
        if (odd == null) {
          semOdd++;
          continue;
        }

        const comGol = partidaComGolNaJanelaEstimativa(p);
        if (p.status === 'em_janela' && comGol !== true) {
          emAberto++;
          continue;
        }
      ...
      return {
        ganho,
        perdas,
        liquido: ganho - perdas,
        vencedores,
        perdedores,
        semOdd,
        emAberto,
        jogosConsiderados: vencedores + perdedores,
      };
    }

    function formatarEstimativaMoeda(valor, comSinalPositivo) {
      const n = Number(valor);
      if (!Number.isFinite(n)) return '—';
      const abs = Math.abs(n).toFixed(2);
      if (n < 0) return '-' + abs;
      if (comSinalPositivo && n > 0) return '+' + abs;
      return abs;
    }

    function renderEstimativaLucroFutebol(est) {
      if (!est) return '';
      const clsLiquido = est.liquido >= 0 ? 'positivo' : 'negativo';
      const extras = [];
      if (est.emAberto > 0) extras.push(est.emAberto + ' em aberto');
      if (est.semOdd > 0) extras.push(est.semOdd + ' sem ODD na 1ª coleta');
      const extrasTxt = extras.length
        ? '<p style="color:#888;font-size:11px;margin:8px 0 0">' + escapeHtml(extras.join(' · ')) + '</p>'
        : '';
      const semJogos = est.jogosConsiderados === 0
        ? '<p style="color:#888;font-size:12px;margin:4px 0 0">Nenhum jogo com resultado fechado e ODD para estimar.</p>'
        : '';
      return '<div class="futebol-estimativa-lucro">' +
        ...
        '<p><strong>Perda (com gol na janela):</strong> ' +
          '<span class="negativo">' + escapeHtml(formatarEstimativaMoeda(-est.perdas, false)) + '</span>' +
          ' <span style="color:#888;font-size:11px">(' + est.perdedores + ' jogos, −R$ 1 cada)</span></p>' +
        '<p><strong>Estimativa líquida:</strong> ' +
          '<span class="' + clsLiquido + '">' + escapeHtml(formatarEstimativaMoeda(est.liquido, true)) + '</span>' +
          ' <span style="color:#888;font-size:11px">(' + est.jogosConsiderados + ' jogos considerados)</span></p>' +
        semJogos +
        extrasTxt +
      '</div>';
    }

    function aplicarFiltroFutebol(payload, filtros) {
      const maxGols = filtros?.maxGols ?? null;
      const oddMinima = filtros?.oddMinima ?? null;
      if (maxGols == null && oddMinima == null) return payload;
      ...
      const historicoFiltrado = historico.filter((p) =>
        partidaPassaFiltrosFutebol(p, leiturasMap[p.id] ?? [], { maxGols, oddMinima }),
      );
    ...
    function renderFutebolFiltros(filtroMeta) {
      const opcoesGols = [null, 1, 2, 3, 4, 5, 6, 7, 8];
      const botoesGols = opcoesGols.map((max) => {
        ...
        return '<button type="button" class="futebol-filtro-btn futebol-filtro-btn-gols' + (ativo ? ' ativo' : '') + '" ' +
          'data-max-gols="' + escapeHtml(dataMax) + '">' + escapeHtml(rotulo) + '</button>';
      }).join('');
      const opcoesOdd = [null, 1.2, 1.5, 2, 2.5, 3];
      const botoesOdd = opcoesOdd.map((min) => {
        const ativo = futebolFiltroOddMinima === min
          || (min == null && futebolFiltroOddMinima == null);
        const rotulo = min == null ? 'Todos' : '>' + min;
        const dataMin = min == null ? '' : String(min);
        return '<button type="button" class="futebol-filtro-btn futebol-filtro-btn-odd' + (ativo ? ' ativo' : '') + '" ' +
          'data-odd-min="' + escapeHtml(dataMin) + '">' + escapeHtml(rotulo) + '</button>';
      }).join('');
      const oddCustom = futebolFiltroOddMinima != null
        && !opcoesOdd.includes(futebolFiltroOddMinima)
        ? String(futebolFiltroOddMinima)
        : '';
      return '<div class="futebol-filtro-max-gols">' +
        '<label for="futebol-filtro-max-gols-opcoes">Máx. gols antes da janela (aos 85\')</label>' +
        '<div class="futebol-filtro-opcoes" id="futebol-filtro-max-gols-opcoes">' + botoesGols + '</div>' +
        '<label for="futebol-filtro-odd-opcoes" style="margin-top:12px">ODD manter placar aos 85\' maior que</label>' +
        '<div class="futebol-filtro-opcoes" id="futebol-filtro-odd-opcoes">' + botoesOdd + '</div>' +
        '<div class="futebol-filtro-odd-custom">' +
          '<label for="futebol-filtro-odd-custom">Outro valor:</label>' +
          '<input type="number" id="futebol-filtro-odd-custom" step="0.01" min="1" placeholder="ex. 1.85" value="' + escapeHtml(oddCustom) + '">' +
          '<button type="button" id="futebol-filtro-odd-aplicar">Aplicar</button>' +
        '</div>' +
        '<p class="futebol-filtro-hint">' + escapeHtml(montarHintFiltrosFutebol(filtroMeta)) + '</p>' +
      '</div>';
    }

    function salvarFiltroOddMinima(valor) {
      futebolFiltroOddMinima = valor;
      if (futebolFiltroOddMinima == null) {
        localStorage.removeItem(CHAVE_FUTEBOL_FILTRO_ODD_MIN);
      } else {
        localStorage.setItem(CHAVE_FUTEBOL_FILTRO_ODD_MIN, String(futebolFiltroOddMinima));
      }
    }

    function wireFutebolFiltros() {
      elConteudo.querySelectorAll('.futebol-filtro-btn-gols').forEach((btn) => {
        btn.addEventListener('click', () => {
      ...
      elConteudo.querySelectorAll('.futebol-filtro-btn-odd').forEach((btn) => {
        btn.addEventListener('click', () => {
          const raw = btn.getAttribute('data-odd-min');
          const novo = raw === '' || raw == null ? null : Number(raw);
          salvarFiltroOddMinima(Number.isFinite(novo) ? novo : null);
          fecharGolsMinutoPopovers();
          if (ultimoPayloadFutebol) renderEstatisticasFutebol(ultimoPayloadFutebol);
        });
      });
      const inputOdd = elConteudo.querySelector('#futebol-filtro-odd-custom');
      const btnAplicarOdd = elConteudo.querySelector('#futebol-filtro-odd-aplicar');
      if (btnAplicarOdd && inputOdd) {
        const aplicarCustom = () => {
          const raw = String(inputOdd.value ?? '').trim().replace(',', '.');
          if (!raw) {
            salvarFiltroOddMinima(null);
          } else {
            const n = Number(raw);
            if (!Number.isFinite(n) || n <= 0) return;
            salvarFiltroOddMinima(n);
          }
          fecharGolsMinutoPopovers();
          if (ultimoPayloadFutebol) renderEstatisticasFutebol(ultimoPayloadFutebol);
        };
        btnAplicarOdd.addEventListener('click', aplicarCustom);
        inputOdd.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') aplicarCustom();
        });
      }
    }
    ...
    function renderResumoHistoricoJanela(resumo, golsPorMinutoJogos, numerosPorPartida, estimativaLucro) {
      const jogosMap = golsPorMinutoJogos ?? {};
      const minutosComGol = Object.keys(resumo.golsPorMinuto)
        .map(Number)
        .filter((m) => (resumo.golsPorMinuto[m] ?? 0) > 0)
        .sort((a, b) => a - b);
      const maxExibir = minutosComGol.length
        ? Math.max(87, minutosComGol[minutosComGol.length - 1])
        : 87;
      const linhasMinuto = [];
      for (let m = 85; m <= maxExibir; m++) {
        const n = resumo.golsPorMinuto[m] ?? 0;
        const rotulo = n === 1 ? '1 gol' : n + ' gols';
        if (n > 0) {
          linhasMinuto.push(
            '<span class="futebol-gol-minuto-wrap">' +
              '<button type="button" class="futebol-gol-minuto futebol-gol-minuto-btn com-gol" ' +
                'data-minuto="' + m + '" aria-expanded="false" aria-haspopup="true" ' +
                'title="Ver jogos com gol aos ' + m + ' min">' +
                escapeHtml(m + "' — " + rotulo) +
              '</button>' +
          ...
          linhasMinuto.push(
            '<span class="futebol-gol-minuto">' + m + "' — " + escapeHtml(rotulo) + '</span>',
          );
        }
      }
      const minutosHtml = linhasMinuto.length
        ? '<div class="futebol-gols-por-minuto">' + linhasMinuto.join('') + '</div>'
        : '<p style="color:#888;font-size:12px;margin:4px 0 0">Nenhum gol registrado em futebol_eventos_gol ainda (coletas após deploy).</p>';
      return '<div class="futebol-historico-resumo-stats">' +
        '<h4>Resumo da janela final</h4>' +
        '<p style="color:#666;font-size:11px;margin:0 0 8px">Fonte: banco (futebol_partidas + futebol_eventos_gol)</p>' +
        ...
        '<p><strong>Gols por minuto de jogo:</strong>' +
          ' <span style="color:#888;font-size:11px">(clique no minuto com gol para ver os jogos)</span></p>' +
        minutosHtml +
        renderEstimativaLucroFutebol(estimativaLucro) +
      '</div>';
    }
    ...
    function renderTabelaLeiturasPartida(partida, leituras) {
      ...
      const linhas = leituras.map((l, idx) => {
        ...
        const maisGols = formatarDeltaGolsLeitura(
        ...
        const tempoJogo = l.minuto_jogo != null
          ? l.minuto_jogo + "'"
          : (l.minuto_relogio?.trim() || '—');
        return '<tr>' +
          '<td>' + escapeHtml(formatarDataComSegundos(l.coletado_em)) + '</td>' +
          '<td>' + escapeHtml(tempoJogo) + ' <span style="color:#666">(' + escapeHtml(placar) + ')</span></td>' +
    ...
    function renderBlocoPartidaHistorico(partida, leituras, expandido, numeroLinha) {
      ...
      const rotuloGolsAntes = golsAntes != null
        ...
        : '';
      const oddInicio = oddManterInicioJanela(partida, leituras);
      const rotuloOddInicio = oddInicio != null
        ? '<span class="futebol-odd-inicio-janela">· ODD ' + escapeHtml(oddInicio.toFixed(2)) + '</span>'
        : '';
    ...
    let golsMinutoPopoverDocBound = false;

    function fecharGolsMinutoPopovers() {
      elConteudo.querySelectorAll('.futebol-gol-minuto-popover').forEach((pop) => {
        pop.classList.add('hidden');
      });
      elConteudo.querySelectorAll('.futebol-gol-minuto-btn').forEach((btn) => {
        btn.setAttribute('aria-expanded', 'false');
      });
    }

    function wireGolsPorMinutoPopover() {
      elConteudo.querySelectorAll('.futebol-gol-minuto-btn').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          const wrap = btn.closest('.futebol-gol-minuto-wrap');
          const pop = wrap?.querySelector('.futebol-gol-minuto-popover');
          if (!pop) return;
      ...
      if (!golsMinutoPopoverDocBound) {
        golsMinutoPopoverDocBound = true;
        document.addEventListener('click', () => fecharGolsMinutoPopovers());
      }
    }
    ...
    function renderTabelaHistoricoFutebol(historicoJanela, leiturasPorPartida, resumoJanela, golsPorMinutoJogos, filtroMeta) {
      ...
      if (!lista.length) {
        const filtroTxt = mensagemHistoricoVazioFiltro(filtroMeta);
        return '<h3 class="futebol-secao-titulo">Histórico — janela final (85\' até o fim)</h3>' +
          renderFutebolFiltros(filtroMeta) +
      ...
      const estimativaLucro = calcularEstimativaLucroFutebol(lista, leiturasPorPartida);
      ...
      return '<h3 class="futebol-secao-titulo">Histórico — janela final (85\' até o fim)</h3>' +
        renderFutebolFiltros(filtroMeta) +
        renderResumoHistoricoJanela(resumo, golsPorMinutoJogos, numerosPorPartida, estimativaLucro) +
    ...
    function renderEstatisticasFutebol(payload) {
      const filtros = { maxGols: futebolFiltroMaxGols, oddMinima: futebolFiltroOddMinima };
      const filtrado = aplicarFiltroFutebol(payload, filtros);
      const {
      ...
      if (filtros.maxGols != null || filtros.oddMinima != null) {
        aoVivo = aoVivo.filter((j) => jogoAoVivoPassaFiltros(j, partidas, leiturasPorPartida ?? {}, filtros));
      }
