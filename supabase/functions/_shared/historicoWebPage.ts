...
<body>
  ...
  <script type="module">
    ...
    function blocoPeriodoTempo(periodo, tempo, estado) {
      ...
      if (estado === 'ao_vivo' && tempo) return p + ' [ ' + tempo + ' ]';
      return p;
    }
    ...
    function formatarDetalhePeriodo(e, estado) {
      const bloco = blocoPeriodoTempo(
        periodoValido(e.periodo) ? e.periodo.trim() : 'Finalizado',
        e.tempoRestante,
        estado,
      );
      if (e.rotuloVantagem && e.rotuloVantagem !== 'empate') {
        return bloco + ' ' + e.rotuloVantagem;
      }
      if (e.rotuloVantagem === 'empate') return bloco + ' empate';
      return bloco;
    }

    function renderTimeline(entrada, timeCasa, timeFora, estado) {
      ...
      return '<div class="timeline-item">' +
        ...
        '<div class="timeline-conteudo">' +
          ...
          '<div class="timeline-detalhe">(' + escapeHtml(formatarDetalhePeriodo(entrada, estado)) + ')</div>' +
        '</div></div>';
    }
    ...
    function renderCard(jogo) {
      ...
      const timeline = exp
        ? '<div class="timeline">' + jogo.entradas.map((e) => renderTimeline(e, jogo.timeCasa, jogo.timeFora, jogo.estado)).join('') + '</div>'
        : '';
