...
export const PAINEL_BUILD_ID = 'futebol-gols-minuto-fix-20260707';
  ...
  <script type="module">
    ...
    function extrairMinutoJogo(texto) {
      ...
      if (clock) {
        ...
        // Relógio decorrido (ex.: 88:32)
        if (m >= 46) return m;
        // Tempo restante no 2º tempo (ex.: 4:30 ≈ 86')
    ...
    function deltaGolsLeitura(leitura, anterior, placarCasaInicio, placarForaInicio) {
      const mais = calcularMaisGolsLeitura(leitura, anterior, placarCasaInicio, placarForaInicio);
      if (mais === '—' || mais === '0') return 0;
      const n = Number(String(mais).replace('+', ''));
    ...
    function atribuirGolsPorMinutoPartida(partida, leituras, golsPorMinuto, statsDebug) {
      if (partida.placar_casa_inicio == null || partida.placar_fora_inicio == null) return 0;
      const golsInicio = totalGolsPartida(partida.placar_casa_inicio, partida.placar_fora_inicio);
      const final = placarFinalPartida(partida, leituras);
      if (!final) return 0;
      const golsTotal = Math.max(0, totalGolsPartida(final.casa, final.fora) - golsInicio);
      if (golsTotal <= 0) return 0;

      if (!leituras.length) return golsTotal;

      let atribuidos = 0;
      const min0 = extrairMinutoJogo(leituras[0].minuto_relogio);
      const abertura = totalGolsPartida(leituras[0].placar_casa, leituras[0].placar_fora) - golsInicio;
      if (abertura > 0 && min0 != null && min0 >= 85) {
        somarGolsPorMinuto(golsPorMinuto, min0, abertura);
        atribuidos += abertura;
        if (statsDebug) statsDebug.viaAbertura += abertura;
      }

      for (let i = 1; i < leituras.length; i++) {
        const min = extrairMinutoJogo(leituras[i].minuto_relogio);
        const delta = totalGolsPartida(leituras[i].placar_casa, leituras[i].placar_fora)
          - totalGolsPartida(leituras[i - 1].placar_casa, leituras[i - 1].placar_fora);
        if (delta > 0 && min != null && min >= 85) {
          somarGolsPorMinuto(golsPorMinuto, min, delta);
          atribuidos += delta;
          if (statsDebug) statsDebug.viaConsecutivo += delta;
        }
      }

      const ult = leituras[leituras.length - 1];
      const minUlt = extrairMinutoJogo(ult.minuto_relogio);
      const fechamento = totalGolsPartida(final.casa, final.fora)
        - totalGolsPartida(ult.placar_casa, ult.placar_fora);
      if (fechamento > 0 && minUlt != null && minUlt >= 85) {
        somarGolsPorMinuto(golsPorMinuto, minUlt, fechamento);
        atribuidos += fechamento;
        if (statsDebug) statsDebug.viaFechamento += fechamento;
      }

      const naoAtribuidos = golsTotal - atribuidos;
      if (naoAtribuidos > 0 && statsDebug) {
        statsDebug.naoAtribuidos += naoAtribuidos;
        statsDebug.amostrasNaoAtrib.push({
          partida: partida.time_casa + ' x ' + partida.time_fora,
          golsTotal,
          atribuidos,
          naoAtribuidos,
          tempos: leituras.slice(-3).map((l) => l.minuto_relogio),
        });
      }
      return golsTotal;
    }
    ...
    function calcularResumoHistoricoJanela(historicoJanela, leiturasPorPartida) {
      ...
      for (const p of historicoJanela) {
        ...
        const golsPartida = atribuirGolsPorMinutoPartida(p, leituras, golsPorMinuto, statsDebug);
        if (golsPartida > 0) jogosComGol += 1;
        totalGolsNaJanela += golsPartida;
      }

      for (const m of Object.keys(golsPorMinuto)) {
        const min = Number(m);
      ...
      fetch('http://127.0.0.1:7904/ingest/86615625-6ae5-4e98-a1da-0a5f0f15fc42', {
        ...
        body: JSON.stringify({
          sessionId: '94b3c3',
          runId: 'gols-minuto-fix',
          hypothesisId: 'H1-H5',
          location: 'historicoWebPage.ts:calcularResumoHistoricoJanela',
          message: 'gols por minuto agregado',
          data: {
            totalGolsNaJanela,
            golsPorMinuto,
            statsDebug,
            maxMinutoComGol: resumo.maxMinutoComGol,
          },
          timestamp: Date.now(),
        }),
      }).catch(() => {});
    ...
    function renderResumoHistoricoJanela(resumo) {
      ...
      for (let m = 85; m <= maxExibir; m++) {
        ...
        const cls = n > 0 ? 'futebol-gol-minuto com-gol' : 'futebol-gol-minuto';
