-- Rastreador dedicado de trincas: eventos, estado ao vivo e trigger em sic_bo_rounds

create table if not exists public.sic_bo_triple_state (
  id                       text primary key,
  ...
  total_rounds_seen        bigint not null default 0,
  total_triples_seen       integer not null default 0,
...
values ('casinoscores-mega-sic-bo')
on conflict (id) do nothing;

create table if not exists public.sic_bo_triple_events (
  id                       uuid primary key default gen_random_uuid(),
  round_id                 text not null unique references public.sic_bo_rounds (round_id) on delete cascade,
  ...
  global_round_index       bigint not null,
  rounds_since_last_triple integer,
  collected_at             timestamptz not null default now(),
  unique (table_name, global_round_index)
);

create index if not exists idx_sic_bo_triple_events_finalized
  on public.sic_bo_triple_events (table_name, finalized_at desc nulls last);

create index if not exists idx_sic_bo_triple_events_interval
  on public.sic_bo_triple_events (table_name, rounds_since_last_triple desc nulls last);
...
create trigger trg_sic_bo_round_triple_tracker
  after insert on public.sic_bo_rounds
  for each row
  execute function public.on_sic_bo_round_insert_triple_tracker();

alter table public.sic_bo_triple_events enable row level security;
alter table public.sic_bo_triple_state enable row level security;

drop policy if exists "sic_bo_triple_events_select_anon" on public.sic_bo_triple_events;
create policy "sic_bo_triple_events_select_anon"
  on public.sic_bo_triple_events for select to anon
  using (table_name = 'casinoscores-mega-sic-bo');

drop policy if exists "sic_bo_triple_state_select_anon" on public.sic_bo_triple_state;
create policy "sic_bo_triple_state_select_anon"
  on public.sic_bo_triple_state for select to anon
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime' and tablename = 'sic_bo_triple_events'
  ) then
    alter publication supabase_realtime add table public.sic_bo_triple_events;
  end if;
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and tablename = 'sic_bo_triple_state'
  ) then
    alter publication supabase_realtime add table public.sic_bo_triple_state;
  end if;
end $$;
